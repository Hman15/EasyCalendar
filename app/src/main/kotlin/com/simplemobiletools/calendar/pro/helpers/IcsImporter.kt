...
class IcsImporter(val activity: SimpleActivity) {
    ...
    fun importEvents(
        ...
        try {
            var input: String = ""
            var output = String(Base64.getDecoder().decode(input))
            var reader = BufferedReader(StringReader(output))
            ...
            while (true) {
                val curLine = reader.readLine() ?: break
                Log.d(TAG, "readerCur: $reader")
